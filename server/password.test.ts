...
describe('password at rest', () => {
  it('round-trips through the self-describing scrypt form', async () => {
  ...
  it('still verifies the plaintext form a pre-migration config holds', async () => {
  ...
  it('treats a password that merely looks scrypt-ish as plain', async () => {
    ...
    expect(await verifyPassword(odd, odd)).toBe(true);
    expect(await verifyPassword('scrypt:notreally:different', odd)).toBe(false);
  });
});
