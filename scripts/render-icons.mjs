...
/** The app's background and NOTHING else — no knight, deliberately.
    There used to be one, centred like a native launch storyboard, and it
    could not be made to leave the screen cleanly: iOS cross-dissolves
    the startup image into the page's first paint, so a knight here fades
    out over whatever the page first shows, and a knight drawn by the
    PAGE to receive the dissolve moves with the still-settling launch
    viewport (both measured on lanph3re's recordings, 2026-08). A plain
    colour dissolving into the same plain colour is the only version of
    this handover with nothing to see. The launch's branding is the app
    icon the OS zooms in from — which is already on screen. */
const splashSvg = (w, h, { bg }) => `<?xml version="1.0"?>
