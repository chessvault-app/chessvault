...
> 측정하는지입니다. 읽는 로직 자체는 공유 코드(`shared/bookImport.ts`,
> `bookConfigSearch.ts`, `bookSolve.ts`, `bookGlyphs.ts`, `bookRepair.ts`,
> `bookEngine.ts`)라서 둘이 서로 어긋날 수 없습니다.

저 문서가 가져오기 단계들과 그 단계가 실행하는 모든 스크립트의 색인이고, 이
파일은 모델 쪽입니다. 둘 사이에는 자기 파일 머리글에 설명이 있는 도구 셋이
있습니다. `search-config.ts`는 책의 표기 설정을 그 책이 인쇄한 정답에서
알아내고, `read-anchors.ts`는 주석 달린 기보집이 인쇄한 다이어그램을 읽어
게임을 포지션에 고정하고, `filter_agree.py`는 학습 전에 의사 라벨을 모델
간 합의로 걸러 냅니다.

## 글자 쪽 모델 (1단계: 번호 라벨 — `digit_labels.py`)

PDF 텍스트 레이어는 1001개 퍼즐 번호 중 83개를 잃어버리고(그 레이어가 정답을
OCR한 결과는 깨진 피겨린 기호 덩어리입니다), 그래서 글자는 책 자신에게서
...
2단계는 이미 반영되어 있습니다(`figurine_glyphs.py`). 검증된 항목이 인쇄된
토큰을 이미 아는 SAN과 정렬해 주므로 단어 상자를 통해 1,600개가 넘는 피겨린
글리프 조각에 라벨을 붙입니다(5-폴드 99.7%). 모든 접두사 단어를 읽으면
204개의 접두사→기물 힌트가 나오고, `autoimport-measure --glyph-hints`가 이를
3패스로 적용합니다. 글자만으로 배운 표기 방식보다 검증이 24개 늘었습니다.
...
가운데 2칸, 12개 가운데 3칸). 책의 라인 전체가 재생되는 유일한 포지션만
받아들입니다. 검증이 30개 더 늘었고, 후보가 여럿인 경우는 건드리지
않았습니다.
...
브라우저에서 실행됩니다. 배포에는 이 중 어느 것도, GPU도 필요하지 않습니다.
...
- **tf-env** (Python 3.10):
  ...
  linrock 교사 모델을 실행하고 PDF 렌더 도우미를 쓸 때만 필요합니다.

## 데이터 출처 (전부 `data/ml/`에 저장되며, git에서 제외됨)
...
2. **인쇄 글꼴 합성** — `gen_print.py`가 고전 다이어그램 글꼴로 무작위
   ...
   포지션을 렌더링하므로, 대응을 바꾼 뒤에는 눈으로 확인합니다.
3. **의사 라벨을 붙인 실제 책** (chessvision식 플라이휠) —
   ...
   워프를 페이지에 적용하며, `pseudo_label.py`가 교사 모델이 0.999 이상의
   확신으로 판정한 타일만 남깁니다(빈 칸은 보드당 개수를 제한합니다). 글꼴,
