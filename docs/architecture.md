...
- **PWA**: the same web app installed from the browser. Manifest +
  ...
  handling for notches, a launch screen (installed apps only — see
  `web/index.html`), theme-aware startup images generated by
  `scripts/render-icons.mjs`, and a code-split landing chunk because iOS
