...
브라우저에서 목록으로 보고 바꿔 가며 검색합니다. 그 관리자에서 PGN 모음을
올리고(여기의 모든 데이터베이스 빌드가 쓰는 `vault/sources/` 업로드입니다),
골라서 이름 붙여 색인합니다. 데이터베이스 페이지에서는 필요 없어진 것을 — 만든
데이터베이스든 그것을 만든 업로드든 — 지웁니다. 그래서 퍼즐과 달리 교체가
번거롭지 않습니다. 같은 이름으로 다시 만들면 교체되고, 새 이름으로
...
방식입니다), 엘리트 브라우저의 만들기 제안은 파일을 넣는 두 번째 방법을
...
지우기는 — 만든 데이터베이스든 올린 모음이든 — **데이터베이스 페이지**의
것이고, 다른 곳에는 없습니다. 같은 관리자가 엘리트 브라우저 위에 창으로도
열리지만 그 창에는 휴지통이 없습니다. 그 창은 검색 도중에 무언가를 고르거나
더하려고 여는 곳이고, 이 기능에서 유일하게 되돌릴 수 없는 조작이 그렇게 자주
들르는 화면에 있을 이유는 없습니다. 대신 그 창은 지우기가 어디에 있는지 말해
주고, 그리로 가는 링크를 답니다.

지우기가 있는 곳에서는 먼저 묻습니다. `ConfirmSheet`으로 — 데스크톱에서는
가운데 창, 폰에서는 아래에서 올라오는 시트입니다. 어느 쪽도 사본을 남기지
않습니다. 두 경로 어디에도 휴지통 디렉터리가 없으니, 그 질문이 유일하게 사이에
있는 것입니다. 각 질문은 무엇이 *영향받지 않는지*를 말해 줍니다. 사람들이
짐작으로 채우는 부분이기 때문입니다 — 데이터베이스를 지워도 그것을 만든 모음은
남고, 모음을 지워도 이미 만들어진 데이터베이스는 그대로입니다.
