...
그 밖의 모든 것 — 책, 스터디, 노트, 가져온 퍼즐 책 — 은 앱 안에서
...
- **무엇을 담는가.** `vault/games` 아래의 모든 PGN: 모음과, 캐시된 온라인 기보
  ...
  제한), 파일의 변경을 stat으로 알아채 바뀐 파일만 다시 색인합니다 — 온라인 기보
...
- **무엇을 아는가.** 각 게임이 누구의 것인지(온라인 기보 경로, `VaultSide`
  ...
  **60반수까지의** 모든 포지션. 참고 데이터베이스의 두 배 깊이인데,
...
만든 데이터베이스든 그것을 만든 업로드든 — 삭제합니다. 아래의 동작은 모두
...
- 같은 브라우저가 **포지션이나 기물 구성으로도 사냥합니다**. 검색창 옆의
  ...
  보드에서 직접 차려서 — 받고, 얼마나 가깝게 맞출지를 고릅니다: 정확한
  포지션, 같은 폰 배치·같은 기물, 같은 폰 파일·같은 기물, 같은 폰 구조,
  같은 기물 구성 — 고정된 완화
  사다리입니다(왜 쿼리 언어가 아니라 사다리인지는 `docs/deferred.md`에
  기록되어 있습니다). 기물 사냥은 엔드게임 상황을 고릅니다 — 프리셋은
  데이터입니다, `web/src/games/endgames.json`, 폰 엔드게임부터 "퀸 하나
  앞섬"까지 — 아니면 기물별·진영별 개수를 직접 짜는 커스텀 편집기를 쓰고,
  그 구성이 얼마나 유지되어야 하는지(어느 순간이든, 4수 이상, 8수 이상)를
  더합니다. 찾은 게임은 스캔이 도는 동안 진행 상황과 함께 실시간으로
  도착하고, 위의 필터 줄과 검색창은 둘 다 텍스트 검색을 좁히는 것과
  똑같이 사냥을 좁히며, 어느 쪽을 고쳐도 열려 있는 사냥이 다시 실행됩니다.
  탐색기의 **데이터베이스 브라우저에서 이 포지션 찾기** 버튼은 지금 포지션을
  그대로 넘겨서, 미리 채워진 채 이미 실행 중인 상태로 도착합니다.
...
덤프 하나가 아니라 각자 갖고 있는 PGN 파일이라는 것 — 이 해법의 모양을
...
색인으로 찾아 들어가지, 훑어서 찾지 않습니다. 흔한 이름은 그냥 걷습니다 —
...
어디 서 있는지를 단계별로 풉니다 — 정확한 포지션, 같은 폰 배치·같은
기물, 같은 폰 파일·같은 기물, 같은 폰 구조, 같은 기물 구성 — 하지만
차례는 절대 풀지 않습니다. 기물 검색에는 목표
포지션 자체가 없습니다: 기물별 개수 범위, 백 빼기 흑 차이 범위, 그
상황이 몇 반수 동안 유지되어야 하는지뿐. 봉투가 게임을 통째로 건너뛰고, 팩이
...
- **완화 단계와 기물 구성 검색: 0.1–0.8초**, 빠른 검색이 켜져 있을
  ...
  순수 JavaScript로 몇 분에 흐릅니다 — 느릴 뿐, 안 되는 일은 없습니다.
- **텍스트 검색: 28–110 ms.** 아무것도 안 걸리는 경우와 희귀한
  플레이어까지, 조회 테이블과 union-seek을 통해서입니다.

기계 크기를 잡는 어림값:

- **색인**은 키를 뒤집는 동안 포지션당 약 8바이트 — 게임당 대략
  0.7 KB — 를 잠시 쥡니다. 2 GB 서버는 ~200만 게임까지 무리 없이
  색인하고, Gigabase급 자료는 ~8 GB 여유를 원합니다. 나머지는 전부
  스트리밍입니다.
- **빠른 검색**은 팩을 메모리에 쥡니다: 옵트인이 유지되는 동안 게임당
  약 0.5 KB(30분 유휴 시 내려갑니다). 정확 검색에는 필요 없습니다 — RAM이
  있는 곳에서 데이터베이스별로 켜면 됩니다.
- **디스크**는 색인 전부 포함 게임당 약 3 KB — 원본 PGN의 대략
  4배입니다.
- **마운트된 데이터베이스의 재색인**은 WAL 아래에서 작업을 커밋하지만,
  끝의 저널 되접기는 서버의 핸들이 닫혀 있어야 합니다. 패스가
  "database is locked"로 끝났다면 서버를 멈추고 다시 돌리면 됩니다 —
  데이터는 이미 들어가 있습니다.
...
둘 다 먼저 묻습니다. `ConfirmDialog`로 — 데스크톱에서는 가운데 창, 휴대폰에서는
...
만들기가 도는 동안에는 서버가 PGN 파일 삭제를 아예 거절합니다(409). 색인기가 그
