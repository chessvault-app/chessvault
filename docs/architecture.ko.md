...
- **PWA**: 브라우저에서 설치한 같은 웹 앱입니다. 매니페스트 + 서비스
  ...
  로딩됩니다. 타일 여섯 개를 그리자고 시작 페이지가 엔진과 탐색기와 PGN
  파서까지 불러오는 비용을 치러서는 안 되기 때문입니다. 지금 시작 페이지가
  받는 JS는 308 kB입니다 — 셸 216 kB에 번역 사전 90 kB.
...
프로그램은 시작용 세트까지 심어 줍니다). 데스크톱 빌드는 서버가 `/updates`에
호스팅하는 피드에서 갱신되므로, 릴리스에 제3자도 공개 저장소도 필요하지
않습니다.
