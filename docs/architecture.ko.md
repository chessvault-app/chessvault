...
- **PWA**: 브라우저에서 설치한 같은 웹 앱입니다. 매니페스트 + 서비스
  ...
  한국어로 599 kB입니다 — 셸 503 kB(41개 조각: 228 kB의 진입 조각, 129 kB의
  컴포넌트 층, 61 kB의 다이얼로그)에 사전 96 kB — 영어로는 503 kB이고,
  0.5.0 빌드에서 쟀습니다. 컴포넌트 층이 들어오기 전의 셸은 217 kB였고,
  Base UI 이식이 컴포넌트 층과 다이얼로그 조각을 다시 키웠습니다. 한 릴리스
  분량의 새 UI 문구는 사전에만 비용을 물립니다. 0.5.0은 데이터베이스 어휘,
  레벨 구간, 전수 검색, 비교 리포트를 더했지만 셸은 1 kB도 움직이지
  않았습니다.
