...
- **서버** (`server/`, Node 위의 Hono): 보관함 파일에 대한 HTTP API와 빌드된
  ...
  서버나 직접 띄운 서버를 가리킵니다. `tablebaseDir`이 Syzygy 파일이 든
  폴더를 가리키면 서버를 아예 거치지 않고 네이티브 코어의 상주
  테이블베이스 모드가 읽어서 답합니다 — `server/tablebaseNative.ts`)를
  소유합니다. 두 프록시 모두
...
- **작업 자식 프로세스** (서버가 띄우며, 인프로세스가 아님 — 아래 두 가지는
  일부러 둔 예외입니다): 무거운
  ...
  비교, 그리고 푸시마다 CI가 돌리는 무작위 말뭉치 퍼즈로 바이트 단위까지
  묶여 있습니다. 심층 검색 스캔은 실행 중에도 묶여 있어, 서버가 네이티브
  적중 하나하나를 참조 스캐너로 다시 재생한 뒤에야 스트리밍합니다.
  `CHESS_NATIVE=0`은 비교를 위해 JS 경로를 고정합니다. 바이너리는
  ...
  있습니다. 두 번째 예외는 같은 모양을 다른 언어로 한 것입니다.
  `chessvault-core tablebase`는 메모리에 매핑한 Syzygy 파일을 쥐고 있는
  **상주** 자식으로, 한 번 띄워 두고 계속 씁니다. 조회 하나를 위해
...
- **데스크톱** (`desktop/`, Electron): 실행할 때 고르는 두 가지 모드가
  ...
  서버를 로컬 폴더에 대해 띄움). 셸은 아키텍처가 아니라 포장입니다. IPC가
  아주 없지는 않고, 설정 페이지가 있는지 확인하고 쓰는 좁은 선택적
  다리(`window.vaultShell`, desktop/preload.cjs)가 있습니다 — 보관함
  바꾸기, 업데이터, 그리고 보관함이나 테이블베이스 파일 폴더를 고르는
  네이티브 대화 상자입니다. 그 뒤에 있는 것 중 **동작**은 하나도 없고,
  거기서 나온 값도 모두 같은 HTTP API로 서버에 갑니다. 다리가 없는
  브라우저에서는 그 조작들이 그려지지 않을 뿐입니다.
- **PWA**: 브라우저에서 설치한 같은 웹 앱입니다. 매니페스트 + 서비스
  ...
  컴포넌트 층, 63 kB의 다이얼로그)에 사전 99 kB — 영어로는 563 kB이고,
  0.7.2 빌드에서 쟀습니다. 컴포넌트 층이 들어오기 전의 셸은 217 kB였고,
  ...
  예약 모듈 셋, 다시 쓴 판 넘김, 반쯤 더 커진 스켈레톤 파일 — 그리고 그 모두가
  첫 그림을 그리려고 런처가 받아야 하는 코드입니다. 그중 240 → 248 kB를 진입
  조각이 가져갔습니다. 답이 오기 *전에* 벌어지는 일에 자신을 쓴 릴리스는 가장
  먼저 도착해야 하는 조각으로 그 값을 치릅니다.
