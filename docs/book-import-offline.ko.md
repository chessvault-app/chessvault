...
대신하지는 않습니다.** 아래는 같은 가져오기를 터미널에서 모는 방법이고,
수고를 들일 만한 경우는 정확히 셋입니다.

- **다시 가져올 때.** 보드 읽기와 엔진의 답은 디스크에 캐시되므로, 이미 읽은
  책을 두 번째로 도는 것은 몇 분이 아니라 몇 초입니다. 앱은 매번 모든 보드를
  다시 읽습니다.
...
314초입니다. 어느 쪽이든 같은 CellNet이고, 보드 하나의 약 948밀리초가 그
모델입니다. 셸이 이기는 곳은 두 번째 실행입니다. 픽셀을 하나도 읽지 않으니
2초입니다.
...
- **엔진 설치는 필요 없습니다.** 등급 단계는 `npm install`이 가져온
  Stockfish를 씁니다.
- **당신의 PDF.** 여기서 책을 받아 오는 것은 없고, 딸려 오는 책도 없습니다.
...
알아내 주는 도구는 보드가 필요한데, 보드는 단계를 한 번 돌려야 생깁니다.
그래서 대충 하나를 써서 책을 읽고, 책이 당신을 고치게 둡니다.
...
책을 보고 읽어 넣으십시오. 당신이 주는 사실 중 책이 스스로 말해 줄 수 없는
것은 이것뿐입니다.
...
측정 단계가 검증할 수 있는 것을 검증하고, 가져오기 단계가 나머지를 엔진에
비추어 등급 매기고 보관함의 책을 씁니다. 마지막 셋은 근거 이미지를
...
- **보드를 다시 읽으려면 설정의 `cache` 파일을 지우십시오.** 그러지 않으면
  측정은 캐시된 읽기를 가져다 검증만 다시 합니다. 파서를 고친 뒤에는 그것이
  바라는 바이고, 모델을 바꾼 뒤에는 정확히 바라지 않는 바입니다.
