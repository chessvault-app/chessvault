...
자기 것만 가져갑니다. 셋이 **같은** 릴리스에 올라가야 하는데, 그것이
`releaseType: draft`로 사는 것입니다. 몇 분씩 차이를 두고 끝나는 세 개의
...
| | 하는 일 |
| --- | --- |
| `release.sh` | 서버 배포, `/api/health`가 이 버전을 말하는지 확인, 태그 푸시 |
| `desktop` 워크플로 | 윈도우·macOS·리눅스를 하나의 **초안** 릴리스에 빌드 |
| 당신 | 둘 다 확인하고 초안을 공개 |

대상은 `scripts/deploy.env`(`CHESS_VAULT_HOST`)에서 읽고, 작업 트리가
지저분하면 실행을 거부하며, 태그가 이미 있어도 거부합니다 — 빌드를
띄우는 것이 그 태그이므로, 이미 있다는 것은 아무 일도 일어나지 않는다는
뜻이기 때문입니다.

서버 배포와 설치 파일이 묶여 있는 것은 일부러입니다. 원격 모드에서
데스크톱 앱은 **서버의** 웹 빌드를 돌리므로, 배포 없이 설치 파일만 내면
둘이 지금 버전이 무엇인지를 두고 어긋나게 됩니다. 배포가 먼저 가고,
사람이 동의할 때까지 초안이 설치 파일을 붙들어 둡니다 — 예전의 전부
아니면 전무 스크립트가 주던 것과 같은 보장이며, `&&`가 있던 자리에
사람이 있을 뿐입니다.
