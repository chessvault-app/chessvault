...
처음 실행할 때 두 가지 모드 중 하나를 고릅니다. 나중에 바꾸려면 설정의
데스크톱 앱 카드에서 보관함 행의 전환… 버튼을 쓰거나, 제목 표시줄 왼쪽
☰ 메뉴의 보관함 전환…을 씁니다:
...
덕분에 데스크톱 빌드는 언제든 떼어 낼 수 있고, 웹 배포가 기준으로 남습니다.

## 창 재질

Windows 11(22H2 이상)은 창의 크롬 뒤에 Mica를, macOS는 사이드바
비브런시를 그립니다. 셸은 둘 중 하나를 청할 수 있고, 설정의 데스크톱 앱
카드에 스위치가 있습니다. 켜기 전까지는 꺼져 있으며, 바꾸면 창을 다시
읽습니다.
...
1. `desktop/build-server.mjs`: 서버를 esbuild로 묶어
   ...
   검색이 실행되는 상주 스캔 워커(`scan-worker.mjs`)와 데이터베이스마다
   하나씩 두는 쿼리 워커(`query-worker.mjs`)를 그 옆에 만들고,
   better-sqlite3를 복사하며(v13은 Node-API 프리빌드를 제공하므로
   Electron에서도 ABI가 안정적이라 다시 빌드할 필요가 없습니다),
   `icon.ico`를 렌더링합니다.
   ...
   그렇다고 알려 줍니다. 설치 프로그램은 그러면 0.5.0 이전의 모든 릴리스가
   그랬듯 JavaScript 작업을 실행합니다. CI는 러너마다 크레이트를 먼저
   빌드하고, 무엇이든 올리기 전에 바이너리가 있는지 확인합니다.
...
4. `desktop/after-pack.mjs`: `release/server/node_modules`를 패키징된
   ...
   디렉터리는 `filter`에 무엇을 적었든 아무 경고 없이 버려집니다. 0.4.2
   전까지 앱은 데이터베이스 드라이버 없이 배포되었고, 모든 로컬 보관함이 실행
...
Node로(`ELECTRON_RUN_AS_NODE`) 실행하며,
`CHESS_VAULT_DIR`/`CHESS_VAULT_DATA`를
`%APPDATA%/Chess Vault/{vault,data}`로 가리키게 하므로 기기 프로필마다 새
보관함이 생깁니다.
...
켠 데이터베이스가 패키징된 앱 안에서도 압축 스캔 색인을 메모리에 상주시킬
수 있는 것은 그 덕분입니다. 열린 참고 데이터베이스마다 줄을 훑는 문장을
돌리는 자식 프로세스 `query-worker.mjs`도 함께 실립니다.
...
`latest-mac.yml`, `latest-linux.yml`이고, 각 플랫폼의 업데이터는 자기
것만 가져갑니다. 셋이 **같은** 릴리스에 올라가야 하며, 그것을 보장하는 것이
`releaseType: draft`입니다. 몇 분씩 차이를 두고 끝나는 매트릭스 잡 세 개는
...
업데이트가 무엇으로 검사되고 무엇으로는 검사되지 않는지도 적어 둡니다.
설치 파일의 sha512가 `latest.yml`의 값과 맞아야 하고, 둘 다 GitHub에서
HTTPS로 옵니다. 코드 서명은 없습니다. 윈도우 빌드는 서명되지 않았고 macOS
빌드는 임시 서명(`"identity": null`)이라, 그 파일을 누가 만들었는지 기기
쪽에서 보증하는 것이 없습니다. 저장소에 릴리스를 올릴 수 있는 사람이면
설치된 모든 앱에 코드를 보내는 셈이고, 앱은 그것을 뒤에서 받아 종료할 때
설치합니다. 서명 없는 오픈소스 앱의 보통 상태이며, 그래서 게시는
워크플로가 밟는 단계가 아니라 사람이 누르는 버튼입니다.
...
인프라에 닿을 수 있는지에 달리게 되고 다른 사람은 아예 낼 수 없었습니다.
...
어긋납니다. 설정에서 서버 버전과 셸 버전을 따로 보여 주는 이유가 그것입니다.
