...
| `data/refgames.sqlite` | 엘리트 게임 브라우저 | 시작용 세트가 앱과 함께 옵니다. 직접 만들려면 앱 안에서, 또는 `npm run build:refgames` |
...
북보다 큰 북이 나오고, 같은 모음 위에 `npm run build:refgames`를
돌리면 더 큰 게임 데이터베이스가 나옵니다(아래 참고). 이 과정에서
...
**참고 게임도 앱 안에서 만듭니다.** 데스크톱은 심어진 채 시작하고 —
앱이 처음 열리기 전에 설치 프로그램의 시작용 세트가 자리를 잡습니다 —
데이터베이스가 아예 없으면 엘리트 브라우저가 올려 둔 PGN 모음을
색인해 주겠다고 제안합니다. 북 관리자가 오프닝 북을 만드는 그
업로드에서요. 터미널이 편하다면 같은 색인기를 이렇게 돌립니다:

```bash
npm run build:refgames         # vault/sources -> data/refgames.sqlite
```

어느 쪽이든 만들면 있던 것을 이름 바꾸기로 대체하므로, 돌고 있는
서버는 끝날 때까지 옛 파일을 계속 서빙합니다. 새로 만들지 않고
지우기만 하면 그것으로 끝입니다 — 함께 오는 북을 지울 때와 같습니다.
...
데이터베이스를 교체하는 일. 앱의 만들기 제안은 데이터베이스가 없을
