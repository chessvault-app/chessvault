...
것이 PGN, 마크다운, JSON으로 당신이 소유한 폴더 하나에 저장됩니다.
...
- **보드** — Stockfish 18(WASM, 멀티스레드)로 자유롭게 분석합니다.
  ...
  오프닝 탐색기(로컬 데이터베이스 + Lichess)와 기물이 7개 이하로 남으면
  그 위에 붙는 엔드게임 테이블베이스의 정확한 판정, 정확도와 정직한 브릴리언트
...
- **게임** — 손수 고른 모음(스터디처럼 주석을 달 수 있습니다), 월별로 필터와
  함께 넘겨 보는 chess.com / Lichess 기보, 그리고 직접 PGN 가져오기입니다.
- **데이터베이스 검색** — 내 PGN으로 직접 만든 참고 데이터베이스를,
  데스크톱 체스 데이터베이스가 검색하는 방식 그대로 세 가지로 검색합니다.
  누가·무엇을 묻는 **질의 언어** — `player:`, `opponent:`, `opening:`,
  `eco:`, `event:`, `result:`, `year:`, `elo:` — 는 칩과 제안 목록을 갖추고,
  아무것도 맞을 수 없는 질의에는 미리 경고합니다. **포지션으로**: 정확히 —
  다른 수순으로 도달한 게임 포함 — 또는 같은 폰과 기물 구성에서 폰 골격만,
  기물 구성만 남을 때까지 단계별로 풀어서, 스쳐 간 포지션과 자리 잡은
  구조를 가르는 유지 조건과 함께. **기물 상황으로**: 폰 엔딩부터 "퀸 하나
  앞섬"까지의 프리셋, 또는 진영·기물별로 직접 짠 조건. 정확 포지션 검색은
  천만 게임 자료에서도 밀리초 안에 답하고, 풀어 놓은 사냥과 기물 검색도
  빠른 검색을 켜면 0.1초대에 답합니다 — 측정한 비용 표는
...
스캔 색인과 역키 색인까지 한 번에 씁니다 — Elite 한 달치(280,059 게임)의
색인이 네이티브 바이너리로 64초, 순수 JavaScript로 178초, 파일은 약 1 GB가
됩니다. Mega급 자료(1,000만 게임 이상)는 약 한 시간과 ~32 GB가 들고, 그
크기에서도 정확 포지션 검색은 밀리초 안에 답합니다. 데이터베이스의 **빠른
검색**을 켜면(관리자의 그 행에 있는 토글) 스캔 색인이 서버 메모리에
상주하고, 완화 포지션·기물 사냥이 그 천만 게임을 0.1–0.8초에 답합니다 —
끄면 같은 사냥이 네이티브 바이너리로 ~30초, 순수 JavaScript로 몇 분에
흘러갑니다. 크기 급별로 측정한 비용
전체와 각 작업이 요구하는 RAM은
["규모와 하드웨어"](docs/databases.ko.md#규모와-하드웨어)에 있습니다.
