...
성적은 어땠는가 — 로컬 오프닝 탐색기가 읽는 것이 이것입니다.
(레퍼토리 트레이너는 *온라인* Lichess 데이터베이스를 상대하므로,
북이 아니라 토큰을 원합니다.) 함께 오는 북은 최근 Lichess Elite 한
달치를 추려 걸어 놓은 것으로, 12수까지 포지션 29,297개, 1.5MB,
CC0입니다. *참고 게임*은 게임
