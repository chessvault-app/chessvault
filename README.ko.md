...
- **도구** — 대화형 보드들을 묶어 놓은 곳입니다. 분석 **보드**, 포지션
  **편집기**, 오프닝 **탐색기**로 가는 지름길, 그리고 필드를 상대로 오프닝을
  ...
  방식](docs/repertoire.ko.md)). 그리고 **엔드게임 드릴**은 기물 구성
  하나(룩 엔드게임, 퀸 대 룩, 또는 직접 정한 기물 구성)에서 이기는
  포지션을 무작위로 꺼내, 테이블베이스가 가장 끈질기게 버티는 수를 상대로
  이겨 내게 합니다. 모든 수는 테이블의 판정 그대로 채점됩니다.
...
- **인사이트** — 같은 게임들을 합산해 봅니다. 전체·진영별·시간제별 성적,
  오프닝 계열마다 승·무·패 막대가 달린 표, 각 게임이 오프닝 목록을 벗어난
  지점, 월별·요일별 게임 수, 게임이 끝난 방식, 길이별 결과입니다. 앱의
  다른 곳을 쓰는 동안 창 안에서 내 게임을 훑는 엔진 패스가 모든 표에
  정확도를 더하고 수 품질 카드를 붙이며, **데이터베이스와 비교** 카드는
  데이터베이스의 플레이어들이 잘 고르지 않는 수를 내가 둔 포지션을
  나열합니다.
...
`Ctrl/⌘ K` 무엇이든 이름이나 본문으로 열기 · `Ctrl/⌘ B` 사이드바 접기 또는
펼치기 · `?` 이 목록을 앱 안에서.
...
| `data/refgames/*.sqlite` | 데이터베이스 브라우저, 로컬 탐색기, 레퍼토리 트레이너, 오프닝 맵, 인사이트의 데이터베이스 비교 | 시작용 세트가 앱과 함께 옵니다. 더 만들려면 앱 안에서, 또는 `npm run build:refgames` |
