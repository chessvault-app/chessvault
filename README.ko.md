...
게임, 색인)은 모두 파생 데이터라 보관함 바깥에 따로 살고, 언제든 지우고
다시 만들 수 있습니다.
...
- **데이터베이스 검색** — 내 PGN으로 직접 만든 참고 데이터베이스를,
  ...
  구조를 가르는 유지 조건과 함께. **기물 상황으로**: 폰 엔드게임부터 "퀸 하나
...
- **도구** — 대화형 보드들을 묶어 놓은 곳입니다. 분석 **보드**, 포지션
  **편집기**, 오프닝 **탐색기**로 가는 지름길, 그리고 필드를 상대로 오프닝을
  ...
  라인이 북을 벗어나면 자연스럽게 엔진으로 넘깁니다). 같은 필드를 상대로
...
서버를 바라보는 창이 됩니다 — B를 보세요.) 그다음 어떤 폴더를 쓸지 묻습니다:
...
launchd는 아무것도 펼쳐 주지 않습니다 — `~`도, `$HOME`도요 — 그래서 plist의
모든 경로는 절대 경로여야 합니다. 파일을 쓰는 셸이 대신 채워 넣게 하세요.
아래 heredoc에 따옴표가 없는 것도 그래서입니다:
...
돌리고, 데이터베이스 색인을 최신으로 맞추고, 서비스를 재시작한 뒤 다시
...
게임은 게임 탭에서 플레이어·오프닝·ECO·대회·결과·레이팅으로 검색되고 아무
...
만드세요** — 어차피 그것이 원래 길입니다. 데이터베이스 페이지에서 PGN 파일을
...
**참고 게임도 앱 안에서 만들고, 하나가 아니라 여럿입니다.** 데스크톱은 심어진 채
시작하고 — 설치 프로그램의 시작용 세트가 데이터베이스 하나로 자리를 잡습니다
— 데이터베이스 페이지에서 PGN 파일을 올리고 그중 골라 이름 붙인
