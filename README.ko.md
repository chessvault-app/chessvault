...
만들면 서버가 그것을 먼저 씁니다. 28만 판 빌드가 약 180초에서 약 72초로,
데이터베이스 전체 포지션 검색이 약 13초에서 약 1초로 줄고, 메모리는 4분의
1입니다. 없어도 — 새로 받은 저장소, 데모, 테스트 — 모든 것이 이전 그대로
JavaScript로 돌아갑니다. `CHESS_NATIVE=0`은 그 경로를 강제합니다. 릴리스에는
아직 바이너리가 실리지 않습니다.
