...
있습니다. 원하는 것만, 기기마다 만드세요 — 그리고 **거의 전부 앱
안에서 됩니다**. 터미널이 필요한 것은 내 PGN 모음을 참고 게임으로
색인하는 일 하나뿐이고, 그마저도 설치 프로그램에 시작용 세트가 딸려
옵니다. 아래의 `npm run` 명령들은 터미널을 선호할 때의 대안이지
필수가 아닙니다.

| 데이터 | 켜지는 기능 | 만드는 방법 |
| --- | --- | --- |
| `data/puzzles.sqlite` | 퍼즐 트레이너 | 앱 안에서, 또는 `npm run build:puzzles` |
| `data/refgames.sqlite` | 엘리트 게임 브라우저 | 시작용 세트가 앱과 함께 옵니다. 직접 만들려면 `npm run build:refgames` |
...
**참고 게임은 데스크톱에서는 심어진 채 시작합니다** — 앱이 처음 열리기
전에 설치 프로그램의 시작용 세트가 자리를 잡습니다. 직접 만드는 일은
여전히 셸의 몫인데, 입력이 공개 덤프 하나가 아니라 각자 갖고 있는 PGN
모음이기 때문입니다:
...
만들면 시작용 세트를 대체합니다(이름 바꾸기로 갈아 끼우므로, 돌고 있는
...
다시 만드는 방법과, 왜 참고 게임을 직접 만드는 일만 아직 터미널을
필요로 하는지는 [docs/databases.md](docs/databases.md)에 있습니다.
