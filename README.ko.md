...
**북 하나는 앱과 함께 옵니다.** 새로 설치하면 Lichess Elite 한 달치를
추려 걸어 놓은 북이 이미 들어 있습니다 — 12수까지 국면 29,297개, 1.5MB,
CC0입니다. 덕분에 탐색기와 레퍼토리 트레이너가 첫 수부터 답을 내놓습니다.
앱을 처음 실행할 때 `data/books/`로 복사되고, 그 뒤로는 평범한 북입니다.
지우거나, 이름을 바꾸거나, 그 위에 새로 만들 수 있습니다. 지우면 그것으로
끝이고 다시 생기지 않습니다.
