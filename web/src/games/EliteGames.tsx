...
export function RefDbManager({
  ...
  databases: RefDb[];
  onChanged: () => void;
  /** `stack` fits the browser's sheet; `grid` composes the Databases
  ...
  useEffect(() => {
    const tick = async (): Promise<void> => {
      try {
        const s = (await (
          await fetch("/api/refgames/build/status")
        ).json()) as {
          running: boolean;
          exitCode?: number | null;
          log?: string[];
    ...
    for (const file of Array.from(files)) {
      ...
      try {
        const res = await fetch(
          `/api/sources?name=${encodeURIComponent(file.name)}`,
          {
            method: "POST",
            body: file,
          },
  ...
  const del = async (dbName: string): Promise<void> => {
    setError(null);
    const res = await fetch(`/api/refgames/${encodeURIComponent(dbName)}`, {
      method: "DELETE",
    });
    if (res.ok) onChanged();
    else {
  ...
  const dbListBlock = (
    <div className="flex flex-col gap-1">
      ...
      {databases.length === 0 ? (
        ...
        <RefDbList databases={databases} onDelete={(n) => void del(n)} />
      )}
    </div>
  );
    ...
    <ul className="divide-line border-line divide-y rounded-md border">
      {databases.map((d) => (
        <li key={d.name} className="flex items-center gap-2 py-1 pl-2.5 pr-1">
          <span
            className="text-fg min-w-0 flex-1 truncate font-medium"
            title={d.sources}
          >
            {d.name}
          </span>
 ...
 */
export function EliteGames({
  variant = "window",
}: {
  variant?: "page" | "window" | "column";
}) {
   ...
   */
  const [structured, setStructured] = useState<StructuredFilters>(
    EMPTY_STRUCTURED_FILTERS,
  );
  ...
  const search = useCallback(
    async (q: string, offset: number, db: string | null) => {
      ...
      try {
        ...
        if (st.player && st.outcome !== "any")
          params.set("outcome", st.outcome);
  ...
  useEffect(() => {
    void loadCollection()
      .then((games) =>
        setCollectionKeys(
          new Set(games.map((g) => `${g.white}|${g.black}|${g.date}`)),
        ),
      )
      .catch(() => {});
  }, []);
  const inCollection = (g: RefGame): boolean =>
    added.has(refGameKey(g.id)) ||
    collectionKeys.has(`${g.white}|${g.black}|${g.date ?? ""}`);
  ...
  const showPreview = async (
    ...
    if (!fen) {
      ...
      try {
        ...
        if (!first) return;
        const lastId =
          mainlineFrom(first.tree, first.tree.rootId).at(-1) ??
          first.tree.rootId;
    ...
    setPreview({
      ...
      orientation: "white",
      top: Math.min(
        Math.max(rect.top + rect.height / 2 - 92, 8),
        innerHeight - 200,
      ),
    ...
    <>
      {dbs.length > 1 && (
        <Select
          ...
          groups={[
            { options: dbs.map((d) => ({ value: d.name, label: d.name })) },
          ]}
        />
      )}
      <Button
            ...
            <Field label="Result and strength">
              <div className="flex gap-2">
                <ResultSelect
                  ...
                  onChange={(result) =>
                    setQuickDraft((d) => ({ ...d, result }))
                  }
                />
                <StrengthSelect
                  value={quickDraft.minElo}
                  onChange={(minElo) =>
                    setQuickDraft((d) => ({ ...d, minElo }))
                  }
                />
              </div>
            </Field>
    ...
    <>
      ...
      <ul className="divide-line min-h-0 flex-1 divide-y overflow-y-auto [&>li:nth-child(even)]:bg-fg/[0.022]">
        {rows.map((g) => (
          <li
            key={g.id}
            className="group hover:bg-surface-2 flex items-center gap-1 pr-2 transition-colors duration-100"
          >
            ...
            >
              <span className="min-w-0 flex-1">
                ...
                <span className="text-subtle block truncate text-xs">
                  {g.eco ? (
                    <OpeningTag eco={g.eco} name={g.opening} />
                  ) : (
                    g.opening
                  )}
            ...
            >
              {inCollection(g) ? (
                ...
                <>
                  <Plus
                    className="mr-1 size-3.5 pointer-coarse:size-4.5"
                    strokeWidth={2.5}
                  />
                  {t("Add")}
                </>
  ...
  const body = (
    <>
      ...
      <Panel
        flush
        className={
          page ? "mt-1 min-h-0 flex-1" : "shrink-0 sm:min-h-0 sm:flex-1"
        }
      >
      ...
      <div className="flex shrink-0 items-center gap-2">
        ...
        <h1 className="text-fg min-w-0 flex-1 truncate text-sm font-semibold">
          {(() => {
            ...
            const all = dbs
              ? dbs.reduce((sum, d) => sum + d.games, 0)
              : (meta?.games ?? 0);
            return all
