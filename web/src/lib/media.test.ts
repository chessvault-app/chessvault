...
/**
 * The `wide` variant is declared once in styles/variants.css and mirrored once in
...
describe('useWideLayout mirrors the CSS `wide` variant', () => {
  it('WIDE_MQ equals the media list inside @custom-variant wide', () => {
    const css = readFileSync(fileURLToPath(new URL('../styles/variants.css', import.meta.url)), 'utf8');
    const match = /@custom-variant\s+wide\s*\{\s*@media\s+([^{]+)\{/.exec(css);
    expect(match, 'styles/variants.css declares `wide` as a block-form @custom-variant with an @media inside').not.toBeNull();
