  ...
  useEffect(() => {
    const onChange = (): void => {
      ...
      if (!leaveIsBlocked()) {
        ...
        // A hard cut, on every viewport. The phone crossfaded and slid
        // between pages through the browser's View Transitions for a
        // while; lanph3re read the fade as the screen flickering and
        // had it taken out (2026-09-13).
        setRoute(parse(next));
        return;
      }
      /**
 ...
 * the arrows in the desktop title bar. The same history move the
 * browser's own chrome makes, kept as one call so the callers have
 * a name for it.
 */
