...
import { hasMyFilters, myFilterQuery } from '@/store/explorer';
import {
  bandRows,
  earliestExits,
  endingShares,
...
} from '@/insights/InsightsPage.skeleton';
...
export function InsightsPage() {
  ...
  /** The route's outline drew this same rail and these same cards while
      the chunk came down (lib/lazyRoute); without this the two waits hand
      over through useSlowLoad's 180ms and the column blinks out and back. */
  const [continuing] = useState(routePlaceholderShown);
  const slow = useSlowLoad(report === null && !failed) || (continuing && report === null && !failed);
  const narrowed =
  ...
  return (
    <PageShell width="medium">
      ...
      <InsightsRail filters={filters} onChange={setFilters} onClear={clear} narrowed={narrowed} inert={gated} />
      )}
