...
function Toast({ className, ...props }: ToastPrimitive.Root.Props) {
  return (
    <ToastPrimitive.Root
      data-slot="toast"
      className={cn(
        // `focus-visible:shadow-none` for the reason components/fab.tsx
        // gives at length: this is the other thing in the app that draws
        // a 3px ring in the same band its own `shadow-lg` darkens. F6 puts
        // the keyboard on a toast; measured on the undo toast in light,
        // the ring read a median of 3.55:1 with its lowest quarter at
        // 2.75, because the ground under it was 209 to 240 rather than the
        // page's 245. Flat, it is 3.776 at the median AND at the quarter,
...
function ToastTitle({ className, ...props }: ToastPrimitive.Title.Props) {
  return (
    <ToastPrimitive.Title
      ...
      className={cn("text-sm max-md:type-row font-medium", className)}
      {...props}
    />
  )
}
...
function ToastIcon({ type }: { type: string | undefined }) {
  ...
  return (
    <span
      ...
      className="shrink-0 [&_svg]:pointer-events-none [&_svg:not([class*='size-']):not([class*='glyph'])]:size-4"
    >
      {icon}
    </span>
  )
