...
const tabsListVariants = cva(
  // Coarse pointers: the track lets go of its 32px so the tabs inside can
  // take the app's 36px floor (DESIGN.md, Buttons); the pill's 3px inset
  // is kept, so the strip is 42px there. Fine pointers keep the
  // registry's geometry untouched. Measured before: 25px tabs on the
  // phone's pane strip, the one control every board page turns on.
...
function TabsTrigger({ className, title, ...props }: TabsPrimitive.Tab.Props & { title?: string }) {
  const trigger = (
    <TabsPrimitive.Tab
      ...
      className={cn(
        "relative inline-flex h-[calc(100%-1px)] pointer-coarse:h-9 flex-1 items-center justify-center gap-1.5 rounded-md border border-transparent px-1.5 py-0.5 text-sm max-md:type-row font-medium whitespace-nowrap text-foreground/60 transition-all group-data-vertical/tabs:w-full group-data-vertical/tabs:justify-start hover:text-foreground focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring outline-none disabled:pointer-events-none disabled:opacity-50 has-data-[icon=inline-end]:pr-1 has-data-[icon=inline-start]:pl-1 dark:text-muted-foreground dark:hover:text-foreground group-data-[variant=default]/tabs-list:aria-selected:shadow-sm group-data-[variant=line]/tabs-list:aria-selected:shadow-none [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-']):not([class*='glyph'])]:size-4",
