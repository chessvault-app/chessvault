...
export function StudyView({
  ...
  return (
    <div className={BOARD_HELD_SHELL}>
      ...
      <MobileActionBar>
        <BoardControls className="py-1.5 ios:py-0 ios:justify-evenly ios:gap-0" />
      </MobileActionBar>
    </div>
  );
}
  ...
  return (
    <li className={cn('group flex items-center', sub && 'pl-5')}>
      {renaming === index && coarse ? (
        ...
        <>
          <span className="text-muted-foreground flex h-(--row-h) min-w-0 flex-1 items-center truncate px-1.5 type-row">
            {ownName}
          </span>
          <PromptDialog
        ...
        <button
          ...
          className={cn(
            ...
            'flex h-(--row-h) min-w-0 flex-1 items-center gap-1.5 rounded-md px-1.5 text-left type-row',
